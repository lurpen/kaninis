class TitleScene extends Phaser.Scene {
    ...
    create() {
        let bg = this.add.image(400, 300, 'title');
        // Scale to cover the screen
        let scaleX = 800 / bg.width;
        let scaleY = 600 / bg.height;
        let scale = Math.max(scaleX, scaleY);
        bg.setScale(scale).setScrollFactor(0);

        this.add.text(400, 500, 'Klicka för att starta', {
            fontSize: '32px',
            fill: '#fff',
            backgroundColor: '#000',
            padding: { x: 10, y: 5 }
    ...
    update() {
        ...
        if (pointer.isDown) {
            if (pointer.x < this.scale.width / 3) {
                this.player.setVelocityX(-160);
            } else if (pointer.x > (this.scale.width / 3) * 2) {
                this.player.setVelocityX(160);
